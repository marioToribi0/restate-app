  ...
  return (
    <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-80 relative'>
        ...
        <View className='flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5'>
            ...
            <Text className='text-xs font-rubik-bold text-primary-300 ml-1 pt-1'>{item.rating.toFixed(1)}</Text>
        </View>
...
export const Card = ({onPress, item}: Props) => {
  return (
    <TouchableOpacity  onPress={onPress} className='flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'>
      <View className='flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5 z-50'>
            <Image source={icons.star} className='size-3.5'/>
            <Text className='text-xs font-rubik-bold text-primary-300 ml-0.5 pt-1'>{item.rating.toFixed(1)}</Text>
        </View>
